    ...
    for i in gen_batch(seq, min(batch_size, len(seq)), dynamic_batch=dynamic_batch):
        ...
        for j in range(n_pred):
            pred = sess.run(y_pred,
                            feed_dict={'data_input:0': test_seq, 'keep_prob:0': 1.0,'is_training:0':False})
            if isinstance(pred, list):
                pred = np.array(pred[0])
            test_seq[:, 0:n_his - 1, :, :] = test_seq[:, 1:n_his, :, :]
            test_seq[:, n_his-1, :, :] = pred
            #print('test_seq1:{}'.format(test_seq[:, n_his - 1, :,0]))
            #test_seq[:, n_his - 1, :,0]= np.squeeze(pred)
            #print('test_seq2:{}'.format(test_seq[:, n_his - 1, :,0]))
            #pred=np.expand_dims(pred,axis=-1)
            step_list.append(pred)
        pred_list.append(step_list)
    #  pred_array -> [n_pred, batch_size, n_route, C_0)
    pred_array = np.concatenate(pred_list, axis=1)
    print('pred_array:{}'.format(pred_array.shape))
...
def model_inference(sess, pred, inputs, batch_size, n_his, n_pred, step_idx, min_va_val, min_val):
    ...
    y_val, len_val = multi_pred(sess, pred, x_val, batch_size, n_his, n_pred, step_idx,x_stats)
    labels=x_val[0:len_val, step_idx + n_his, :, 0]
    print('label:{}'.format(labels.shape))
    evl_val = evaluation(labels, y_val, x_stats)
    print('evl_val:{}'.format(evl_val))
    ...
    if sum(chks):
        ...
        y_pred, len_pred = multi_pred(sess, pred, x_test, batch_size, n_his, n_pred, step_idx,x_stats)
        labels=x_test[0:len_pred, step_idx + n_his, :, 0]
        evl_pred = evaluation(labels, y_pred, x_stats)
        min_val = evl_pred
